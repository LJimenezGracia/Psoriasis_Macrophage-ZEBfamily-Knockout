---
title: "Data normalization and pre-processing: DC"
...
# Load Seurat object
seurat_obj <- readRDS(paste0(path_r_objects, "/post12_cd45pos_DC.rds"))
seurat_obj
```
...
openxlsx::write.xlsx(biomarkers_list,
                     file = paste0(path_r_tables, "/biomarkers_cd45pos_DC_resolution-0.1.xlsx"))
```
...
cDC2_genes <- c("Mgl2", "Sirpa", "Itgam", "Irf4", 
...
```{r fig.width=20, fig.height=5}
residentcDC2_genes <- c("Cd163", "Lrp1", "Itgax", "Cd209d")

FeaturePlot(seurat_obj_integrated,
            features = residentcDC2_genes,
            label = T,
            #split.by = "tissue",
            order = F,
            pt.size = 0.25,
...
```{r fig.width=20, fig.height=10}
matureDC_genes <- c("Fscn1", "Cacnb3", "Ccr7", "Cd40", "Tmem123", "Cd274")

FeaturePlot(seurat_obj_integrated,
            features = matureDC_genes,
            label = T,
            #split.by = "tissue",
            order = F,
            pt.size = 0.25,
...
```{r fig.width=20, fig.height=5}
pDC_genes <- c("Ly6d", "Igkc", "Bst2", "Irf8")
...
seurat_obj_integrated <- RenameIdents(
 seurat_obj_integrated,
  ...
  "1" = "Langerhans cells",
  "2" = "Mature DC",
  "3" = "Tissue-resident cDC2",
  "4" = "Doublets DC-T cells",
  "5" = "pDC"
)
...
p1
ggsave(filename = paste0(path_r_figs, "/post12_cd45pos_DC_clustering_annotation_umap.png"),
       plot = p1)
```
...
saveRDS(seurat_obj_integrated@meta.data[c("celltypes_2")],
        paste0(path_r_objects, "/post12_cd45pos_DC_annotations.rds"))

# Save processed lognorm Seurat object
saveRDS(seurat_obj_integrated, paste0(path_r_objects, "/post12_cd45pos_DC_clustering_annotation.rds"))

#seurat_obj_integrated <- readRDS(paste0(path_r_objects, "/post12_cd45pos_DC_clustering_annotation.rds"))
```
