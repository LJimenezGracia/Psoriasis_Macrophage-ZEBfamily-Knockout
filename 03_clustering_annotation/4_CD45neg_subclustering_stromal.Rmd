---
title: "Data normalization and pre-processing: Stromal cells"
...
# Load Seurat object
seurat_obj <- readRDS(paste0(path_r_objects, "/post12_cd45neg_stromalcells.rds"))
seurat_obj
```
...
Idents(seurat_obj_integrated) <- "RNA_snn_res.0.2"
```
...
openxlsx::write.xlsx(biomarkers_list,
                     file = paste0(path_r_tables, "/biomarkers_cd45neg_stromalcells_resolution-0.2.xlsx"))
```
...
```{r fig.width=20, fig.height=5}
chondrocytes_genes <- c("Clu", "Snorc", "Col2a1", "Acan")

FeaturePlot(seurat_obj_integrated,
            features = chondrocytes_genes,
            label = T,
            #split.by = "tissue",
            #order = T,
            pt.size = 0.25,
...
```{r fig.width=20, fig.height=5}
reticularfibro_genes <- c("Angptl7", "Thbs1", "Ccn2", "Mgp")

FeaturePlot(seurat_obj_integrated,
            features = reticularfibro_genes,
            label = T,
            #split.by = "tissue",
            #order = T,
            pt.size = 0.25,
...
innearear_genes <- c("Coch", "Col12a1")

FeaturePlot(seurat_obj_integrated,
            features = innearear_genes,
            label = T,
            #split.by = "tissue",
            #order = T,
            pt.size = 0.25,
...
```{r fig.width=20, fig.height=5}
hairdermal_genes <- c("Enpp2", "Inhba", "Rspo3", "Col23a1")

FeaturePlot(seurat_obj_integrated,
            features = hairdermal_genes,
            label = T,
            #split.by = "tissue",
            #order = T,
            pt.size = 0.25,
...
```{r fig.width=20, fig.height=5}
univfibros_genes <- c("Dpt", "Pi16", "Ly6a", "Ly6c1", "Col14a1")

FeaturePlot(seurat_obj_integrated,
            features = univfibros_genes,
            #split.by = "sample_id",
            #order = T,
            pt.size  = 0.25,
            label = TRUE,
...
```{r fig.width=20, fig.height=5}
vascularsmooth_genes <- c("Apod", "Serpine2", "Clec1a", "Itga6")

FeaturePlot(seurat_obj_integrated,
            features = vascularsmooth_genes,
            label = T,
            #split.by = "tissue",
            #order = T,
            pt.size = 0.25,
...
```{r fig.width=10, fig.height=5}
dermalfibro_genes <- c("Sparcl1", "Col7a1")

FeaturePlot(seurat_obj_integrated,
            features = dermalfibro_genes,
            label = T,
            #split.by = "tissue",
            #order = T,
            pt.size = 0.25,
...
```{r fig.width=20, fig.height=5}
skeletalmuscle_genes <- c("Chodl", "Des", "Nppc", "Pax7", "Dag1")

FeaturePlot(seurat_obj_integrated,
            features = skeletalmuscle_genes,
            label = T,
            #split.by = "tissue",
            #order = T,
            pt.size = 0.25,
...
inflammatory_genes <- c("Ccl7", "Il6", "Ccl2", "Ccl19", "Cxcl12", "Cxcl1", "Ccl11", "Tgfbi")

FeaturePlot(seurat_obj_integrated,
            features = inflammatory_genes,
            label = T,
            split.by = "sample_id",
            #order = T,
            pt.size = 0.25,
...
seurat_obj_integrated <- RenameIdents(
 seurat_obj_integrated,
  "0" = "Reticular fibroblasts",
  "1" = "Universal fibroblasts",
  "2" = "Chondrocytes",
  "3" = "Inner ear fibroblasts",
  "4" = "Universal fibroblasts",
  "5" = "Hair dermal sheath",
...
p1
ggsave(filename = paste0(path_r_figs, "/post12_cd45neg_stromal_clustering_annotation_umap.png"),
       plot = p1)
```

Now, we will also explore clusters across sample.
```{r fig.width=25, fig.height=5}
seurat_obj_integrated$sample_id <- factor(seurat_obj_integrated$sample_id,
...
saveRDS(seurat_obj_integrated@meta.data[c("celltypes_2")],
        paste0(path_r_objects, "/post12_cd45neg_stromal_annotations.rds"))

# Save processed lognorm Seurat object
saveRDS(seurat_obj_integrated, paste0(path_r_objects, "/post12_cd45neg_stromal_clustering_annotation.rds"))
#seurat_obj_integrated <- readRDS(paste0(path_r_objects, "/post12_cd45neg_stromal_clustering_annotation.rds"))
```
