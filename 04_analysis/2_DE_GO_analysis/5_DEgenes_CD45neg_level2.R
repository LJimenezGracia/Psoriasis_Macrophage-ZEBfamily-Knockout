...
## Load data
seurat_obj <- readRDS(paste0(path_r_objects_in, "/post12_cd45neg_alldata_cleaned_final_clustering_annotation.rds"))
...
for (index in 1:length(comparison_name_list)) {
  ...
  path_object <- paste0(path_r_objects_out, "/post12_cd45neg_DEgenes_celltypes_", comparison_name, ".rds")
  path_file <- paste0(path_r_tables, "/post12_cd45neg_DEgenes_celltypes_", comparison_name, ".xlsx")
  path_file_split <- paste0(path_r_tables, "/cd45neg_celltypes/post12_cd45neg_DEgenes_celltypes_", comparison_name, "_UPDOWN.xlsx")
  ...
  seurat_obj_subset$celltypes_2 <- as.factor(as.character(seurat_obj_subset$celltypes_2))
