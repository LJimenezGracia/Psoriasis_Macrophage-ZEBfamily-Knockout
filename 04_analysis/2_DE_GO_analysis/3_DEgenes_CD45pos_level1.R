...
seurat_obj$celltypes_1[seurat_obj$celltypes_1 == "Monocytes/Macrophages"] <- "Monocytes-Macrophages"
seurat_obj$celltypes_1[seurat_obj$celltypes_1 == "T/NK cells"] <- "T-NK cells"
seurat_obj$celltypes_1 <- as.character(seurat_obj$celltypes_1)
...
for (index in 1:length(comparison_name_list)) {
  ...
  path_object <- paste0(path_r_objects_out, "/post12_cd45pos_DEgenes_lineages_", comparison_name, ".rds")
  path_file <- paste0(path_r_tables, "/post12_cd45pos_DEgenes_lineages_", comparison_name, ".xlsx")
  path_file_split <- paste0(path_r_tables, "/cd45pos_lineages/post12_cd45pos_DEgenes_lineages_", comparison_name, "_UPDOWN.xlsx")
  ...
  seurat_obj_subset$celltypes_1 <- as.factor(as.character(seurat_obj_subset$celltypes_1))
